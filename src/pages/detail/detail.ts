...
export class DetailPage {
  ...
  constructor(private alert: AlertController,
   ...
   8
     ...
     selectEntry(item)
     {
        this.technologystart        = item.start;
     ...
     // for the record data
     createEntry(start, end, date, time, sit, brand, color, tel)
     {
        let body     : string   = "key=create&start=" + start + "&end=" + end + "&date=" +
                                   date + "&time=" + time + "&sit=" + sit +
                                    "&brand=" + brand + "&color=" + color + "&tel=" + tel,
            type     : string   = "application/x-www-form-urlencoded; charset=UTF-8",
            headers  : any      = new Headers({ 'Content-Type': type}),
            options  : any      = new RequestOptions({ headers: headers }),
            url      : any      = this.baseURI + "manage-dat.php";
        ...
        .subscribe((data) =>
        {
           ...
           {
              this.hideForm   = true;
              this.sendNotification(`Congratulations the technology: ${start} was successfully added`);
           }
     ...
     // for the record data
     updateEntry(start, end, date, time, sit, brand, color, tel)
     {
        let body       : string = "key=update&start=" + start + "&end=" + end + "&date=" +
                                  date + "&time=" + time + "&sit=" + sit +
     ...
     deleteEntry()
     {
        let name       : string = this.form.controls["start"].value,
    ...
    saveEntry()
     {
        let start          : string    = this.form.controls["start"].value,
            ...
            if(this.isEdited)
            {
               this.updateEntry(start, end, date, time, sit, brand, color, tel);
            }
            else
            {
               this.createEntry(start, end, date, time, sit, brand, color, tel);
            }
         }
